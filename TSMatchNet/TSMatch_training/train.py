...
from model import TSMatchNet
from dataset import AIC22Dataset

def train(save: bool):
    model = TSMatchNet()
    model.cuda()
    model.train()
    dataset = AIC22Dataset(cfg.root_train, 'train')
    ...
    for epoch in range(cfg.train_epoch):
        ...
        loss_sum = 0
        for i, (pair1, pair2, pair3, pair4, pair5, label) in enumerate(dataloader):
            optimizer.zero_grad()
            pairs_1 = torch.cat((pair1[0], pair2[0], pair3[0], pair4[0], pair5[0]), dim=0).cuda()
            pairs_2 = torch.cat((pair1[1], pair2[1], pair3[1], pair4[1], pair5[1]), dim=0).cuda()
...
def validate(model):
    model.eval()
    dataset = AIC22Dataset(cfg.root_train, 'val')
    dataloader = DataLoader(
        dataset=dataset,
...
if __name__ == '__main__':
    ...
    # train(save=True)
    # validation
    model_p = './checkpoints/newmodel_epoch250_tmp.pth'
    model = TSMatchNet()
    model.cuda()
